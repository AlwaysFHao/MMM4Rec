...
def _parse_csv_line(line: str):
    ...
    return {"user": str(data[1]), "item": str(data[0]), "rate": float(data[2]), "time": int(data[3])}
...
def load_inter_file(inter_file):
    ...
    with open(inter_file, "r", encoding="utf-8") as fobj:
        ...
        for line in tqdm(fobj.readlines(), desc=f"load inter file {inter_file}"):
            ...
            # if (user, item, rate, time) not in inters:
            #     inters.add((user, item, rate, time))
                # inters_list.append((user, item, rate, time))
...
def _parse_vision(data):
    ...
    """
    try:
        if "imageURLHighRes" in data and len(data["imageURLHighRes"]) != 0:
            image_url = data["imageURLHighRes"][0]
            if image_url:
                return image_url
        elif "imageURL" in data and len(data["imageURL"]) != 0:
...
def load_meta_file(meta_file):
    ...
    :return: 商品元数据字典{商品id: {"vision": img_url, "text": 商品标题+品牌+文本描述}}
    """
    assert meta_file.endswith(".json.gz"), '请传入正确的商品元数据文件，格式为.json.gz'
    ...
    for line in tqdm(gzip_file, desc=f"load meta file {meta_file}"):
        ...
        # 加入商品元数据字典
        metas[item] = {"vision": vision, "text": text}
    return metas
...
def filter_metas_without_modality(metas, vision_filter=False, text_filter=False):
    ...
    处理缺失模态的商品
    :param metas: 商品元数据
    :param vision_filter: 是否过滤掉无图片的商品
    :param text_filter: 是否过滤掉无文本的商品
    :return: 过滤后的商品元数据
    """
    desc = "filter metas"
