...
from transformers import SiglipModel, SiglipProcessor, RobertaModel, RobertaTokenizer, AutoModel, AutoTokenizer
...
def get_model(vision_name_or_path, text_name_or_path):
    vision_model = SiglipModel.from_pretrained(pretrained_model_name_or_path=vision_name_or_path)
    vision_processor = SiglipProcessor.from_pretrained(pretrained_model_name_or_path=vision_name_or_path)
...
if __name__ == '__main__':
    ...
    api = SiglipRoBertaExtractFeature(data_path, vision_model_name, text_model_name, batch_size=128)
