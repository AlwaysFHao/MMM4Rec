...
class Trainer(object):
    ...
    def _load_pretrain_checkpoint(self, checkpoint):
        ...
        model_checkpoint.pop('fusion_factor', None)
    ...
    def fit(self, train_dataloader, val_dataloader, test_dataloader):
        ...
        for epoch in range(self.max_epochs):
            ...
            with torch.inference_mode():
                if self.fabric.global_rank == 0 and self.is_pbar:
                    pbar = tqdm(total=len(val_dataloader),
                                desc=set_color(f'Validation Epoch: {epoch + 1 + self.start_epoch}', 'pink'),
                                colour='green')
                if torch.cuda.is_available():
                    torch.cuda.synchronize(self.fabric.device)
                    self.fabric.barrier()
                ...
                for batch in val_dataloader:
                    scores_index, batch_num = model.validation_step(batch)
                    validation_step_outputs.append(scores_index)
            ...
            if self.fabric.global_rank == 0 and (self.best_val_score is None or val_score >= self.best_val_score):
                ...
                save_path = os.path.join(self.save_path, 'best_epoch_model.ckpt')
        ...
        if self.fabric.global_rank == 0:
            self.logger.info(set_color('\nThe training is over and the test begins.\n', color='cyan'))
        # 加载最优参数进行测试
        self.test(test_dataloader, os.path.join(self.save_path, 'best_epoch_model.ckpt'))


    def test(self, test_dataloader, checkpoint=None):
        if self.fabric.global_rank == 0:
            self.logger.info("\n-----------------***************** Test *****************-----------------\n")
        if self.fabric.global_rank == 0 and checkpoint is not None:
            self._resume_checkpoint(checkpoint)
        test_dataloader = self.fabric.setup_dataloaders(test_dataloader, use_distributed_sampler=False)
        model = self.model
        model.eval()
        test_step_outputs = []
        batch_count = 0
        with torch.no_grad():
            ...
            for batch in test_dataloader:
                ...
                test_step_outputs.append(scores_index)
                batch_count += batch_num
            ...
            sort_lists = torch.cat(test_step_outputs, dim=0)
            metrics = self.get_metrics(sort_lists, batch_count)
            # metrics = self.fabric.all_reduce(metrics, reduce_op='mean')
        ...
        if self.fabric.global_rank == 0:
            if self.is_pbar:
                pbar.close()
            self.logger.info(set_color(f'Test Time {(td - ts):.5f}s \nTest Metrics: \n{tabulate([list(metrics.values())], headers=list(metrics.keys()), tablefmt="fancy_grid")}', color='cyan'))
