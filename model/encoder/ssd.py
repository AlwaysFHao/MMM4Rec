...
class CoTiSSD(nn.Module):
    def __init__(self,
        ...
        # Order: [z, x, B, dt]
        u_d_in_proj = 2 * self.d_inner + self.d_state + self.n_heads
        self.u_in_proj = nn.Linear(self.d_model, u_d_in_proj, bias=bias)

        # C
        q_d_in_proj = self.d_state
    ...
    def forward(self, q, u, time_diff=None):
        ...
        # d_mlp = d_inner - d_ssm
        d_mlp = (z_x_B_dt.shape[-1] - 2 * self.d_ssm - self.d_state - self.n_heads) // 2

        z0, x0, z, xB, dt = torch.split(
            z_x_B_dt,
            [d_mlp, d_mlp, self.d_ssm, self.d_ssm + self.d_state, self.n_heads],
            dim=-1
        )
...
class CoTiSSDLayer(nn.Module):
    ...
    def forward(self, vision, text, text_time_diff=None, vision_time_diff=None):
        """
        :param vision: shape: [batch_size, seq_len, d_model]
        :param text: shape: [batch_size, seq_len, d_model]
        :param text_time_diff: shape: [batch_size, seq_len]
        :param vision_time_diff: shape: [batch_size, seq_len]
        :return: shape: [batch_size, seq_len, d_model]
        """
        # hidden = self.layer_norm(x)
        hidden, text_time_diff = self.ssd(text, text_time_diff)
        # Determine whether SSDBlock needs residual by num_layers
        hidden = self.layer_norm(self.dropout(hidden) + text)
        # if self.num_layers == 1:
        #     hidden = self.layer_norm(self.dropout(hidden))
        # else:
        #     hidden = self.layer_norm(self.dropout(hidden) + text)
        ...
        co_time_diff = None
        if self.is_time:
            seq_len = text_time_diff.shape[-1]
            temp_text_time_diff = torch.fft.rfft(text_time_diff, dim=2, norm='ortho')
            text_weight = self.text_complex_weight(temp_text_time_diff)
            temp_text_time_diff = text_weight * temp_text_time_diff
            # temp_text_time_diff = self.text_complex_weight(temp_text_time_diff)


            vision_time_diff = torch.fft.rfft(vision_time_diff, dim=2, norm='ortho')
            vision_weight = self.vision_complex_weight(vision_time_diff)
            vision_time_diff = vision_weight * vision_time_diff
            # vision_time_diff = self.vision_complex_weight(vision_time_diff)

            co_time_diff = (temp_text_time_diff + vision_time_diff)
            co_time_diff_weight = self.co_complex_weight(co_time_diff)
            co_time_diff = co_time_diff_weight * co_time_diff


            co_time_diff = torch.fft.irfft(co_time_diff, n=seq_len, dim=2, norm='ortho')

        hidden = self.co_ssd(vision, hidden, co_time_diff)

        hidden = self.co_layer_norm(self.dropout(hidden) + text)

        # if self.num_layers == 1:
        #     hidden = self.co_layer_norm(self.dropout(hidden))
        # else:
        #     hidden = self.co_layer_norm(self.dropout(hidden) + x)

        if self.is_ffn:
            return self.ffn(hidden), text_time_diff
        else:
            return hidden, text_time_diff
