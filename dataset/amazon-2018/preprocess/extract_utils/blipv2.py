...
from transformers import AutoProcessor, Blip2ForImageTextRetrieval, BlipForImageTextRetrieval
try:
...
def get_blip2_dataloader(path, batch_size=64):
...
def get_blip2_model(name_or_path):
    clip_model = Blip2ForImageTextRetrieval.from_pretrained(pretrained_model_name_or_path=name_or_path)
    clip_processor = AutoProcessor.from_pretrained(pretrained_model_name_or_path=name_or_path)
    return clip_model, clip_processor


class Blip2ExtractFeature(object):
    def __init__(self,dataset_path,
                 blip2_model_name_or_path="Salesforce/blip2-itm-vit-g",
                 batch_size=64):
        super(Blip2ExtractFeature, self).__init__()
        self.device = (torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu'))
        self.dataloader = get_blip2_dataloader(dataset_path, batch_size)
        self.blip2_model, self.blip2_processor = get_blip2_model(blip2_model_name_or_path)
    ...
    def __call__(self, feature_save_path):
        with torch.no_grad():
            ...
            for (texts, images) in tqdm(self.dataloader):
                inputs = self.blip2_processor(text=texts,
                                             images=images,
                                             return_tensors='pt',
                                             padding='max_length',
                                             max_length=512,
                                             truncation=True).to(self.device)
                outputs = self.blip2_model(**inputs, use_image_text_matching_head=True)
                # [batch, embedding_dim]
...
if __name__ == '__main__':
    ...
    api = Blip2ExtractFeature(data_path, model_name, batch_size=128)
